...
1. The rover ESP saw the UM982 readiness marker:
   - `$devicename,COM2`
2. GNSS logging worked only when the rover sketch requested logs on `COM2`
   (historically `PVTSLNA COM2 0.1` at 10 Hz; now `PVTSLNA COM2 0.05` at
   20 Hz, plus `RECTIMEA COM2 1` and `UNIHEADINGA COM2 0.2`).
