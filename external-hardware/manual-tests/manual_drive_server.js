// Live manual-drive bring-up server.
// Uses the legacy HID controller mapping, drives the motor node, polls GNSS + IMU,
// and serves a phone-friendly live dashboard over HTTP/SSE.
...
import {
  computeManualDriveDemand,
  manualMotionLimiterOptions,
} from "../../dist/src/control/manualDriveProfile.js";
...
const dashboardPath = path.join(__dirname, "manual_drive_dashboard.html");
const sessionLogPath = path.join(
  __dirname,
  `manual-drive-${new Date().toISOString().replace(/[:.]/g, "-")}.jsonl`,
);
...
function normalizeManualTurnDemand(angleDegrees) {
...
function computeManualDemand() {
  ...
  return computeManualDriveDemand({
    speedDemand: state.controller.speed,
    turnDemand: normalizeManualTurnDemand(state.controller.angleDegrees),
...
async function refreshMotorAndDrive() {
  ...
  const motionLimits = manualMotionLimiterOptions(parameters, CONTROL_LOOP_MS);
  ...
  commandLimiter = new CommandLimiter({
    maxWheelSpeedMetersPerSecond: parameters.maxWheelSpeedMetersPerSecond,
    maxWheelAccelerationStepMetersPerSecond: motionLimits.maxWheelAccelerationStepMetersPerSecond,
...
const server = http.createServer(async (request, response) => {
  try {
    if (request.url === "/" || request.url === "/manual_drive_dashboard.html") {
...
async function main() {
  ...
  controller.on("error", (error) => {
    state.notes.unshift(`Controller error: ${error instanceof Error ? error.message : String(error)}`);
    state.notes = state.notes.slice(0, 8);
  });
  controller.start();

  server.listen(PORT, () => {
    console.log(`Manual drive dashboard ready at http://0.0.0.0:${PORT}`);
