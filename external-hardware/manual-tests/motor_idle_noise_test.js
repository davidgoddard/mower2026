...
// - PASS with note: watchdogHealthy=false is expected if the firmware is
//   currently disabled at zero command
...
function classifyRun(summary) {
  ...
  if (summary.watchdogFaultSamples === summary.sampleCount && summary.sampleCount > 0) {
    notes.push("watchdogHealthy was false for the whole run. That is expected if drive remained disabled at zero command.");
  } else if (summary.watchdogFaultSamples > 0) {
    notes.push("watchdogHealthy was false on some samples. That is usually benign for this idle test unless motion is also reported.");
  }
...
async function main() {
  ...
  try {
    ...
    console.log("Expected watchdog detail: watchdogHealthy may be false while idle.\n");
