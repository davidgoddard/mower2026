...
- when turning: continually monitor the turning and adjust its internal control parameters to reduce errors:
    ...
    - GNSS heading shall not be used or written into the IMU baseline while a turn is active; it may be considered only after motor output is zero and yaw motion has settled
...
- when driving: continually monitor the driving and adjust its internal control parameters to reduce errors:
    ...
    - Every drive should follow a sequence of events:
        ...
        - if the turn component does not report success, stop that segment and do not issue the translation
        - settle 
        ...
        - apply a straight-line drive controller that:
            ...
            - applies proportional and CTE-rate damping left/right wheel trim to keep the mower on the line and suppress repeated baseline crossings
            - keeps both wheel commands in the selected travel direction; it shall never pivot, scrub one wheel, or reverse one wheel after translation has begun
...
- whenever an operator changes a field in an editable settings form, background refreshes shall stop replacing values in that form until the operator saves or cancels the edit; Save shall resume authoritative refreshes using the saved values, while an adjacent Cancel button shall discard the unsaved fields, reload the authoritative values, and resume refreshes. Live status and safety telemetry that does not write into the edited form shall continue updating.
...
- provide a one-off peg-layout utility for the saved Rear Lawn which uses Pattern A's 23-degree long axis, the production-shaped area perimeter, and every saved obstacle exclusion to divide the lawn from the top of the Drive & Paths map into successive bands of approximately 83.25 square metres of usable area. The mower shall segment-drive from its live trusted-GNSS pose to the first upper-left peg point, pause there for 10 seconds, and then alternate across and down the lawn with the same pause at every outer peg point. Transit to each peg shall use the production mowing free-space router and configured mowing standoff; obstacle-routing vertices are transit points and shall not create extra peg pauses. Preview shall be the default, physical execution shall require an explicit command-line flag, and loss of trusted GNSS, a failed segment, an unsafe route, or operator interruption shall stop the run.
...
A sustained GNSS rejection shall log immediately when its primary reason changes and then at most once per minute while that reason remains active. Periodic entries shall include the number of suppressed repeated samples so an outage remains measurable without producing one warning per second.
...
The sensor controller shall detect likely obstructions/stalls from its latest motor feedback, the current motor command and the latest pose estimate. During translational driving, commanded motion must produce meaningful trusted-GNSS positional progress over a short grace-adjusted observation window. During an in-place pivot, commanded motion must instead produce meaningful IMU heading change. Until the installed current sensors have been validated under load, motor current of at least 10 A sustained for approximately two seconds is independently sufficient stall evidence; it must fall below 9.5 A to clear that observation, preventing threshold chatter. On a confirmed stall, the controller shall emit an obstruction event, request a global stop and log the condition once.
...
The ESP32 motor node shall send raw encoder pulse deltas and PWM/current telemetry only. Because the ESP32 and Pi are powered before the separately connected motor battery makes the current sensors stable, current-zero calibration shall not occur at ESP startup. The shared Pi motor-command path shall request calibration before its first non-zero mowing, web-manual, or game-controller command; the ESP shall disable both outputs throughout calibration and report zero current until calibration has completed. A disabled motor command shall mark that calibration stale because the motor battery may be disconnected while the Pi and ESP remain powered, requiring the next drive interaction to calibrate again.
...
**Heading Reset:**
The sensor controller shall support an external absolute heading update (for example from GNSS) which resets the maintained IMU heading to the supplied value. During normal operation, the GNSS heading must already be close to the IMU heading. Every GNSS position or heading value consumed by pose fusion must pass the checks for that individual epoch; a latched TRUSTED state used for promotion/demotion hysteresis shall never make a currently rejected sample eligible for fusion. While the mower is stationary and safe to rebase, five consecutive GNSS samples that independently pass fix, position, heading-validity, heading-accuracy, baseline, and heading-stability checks shall permit a reset regardless of the IMU/GNSS heading difference. GNSS position accuracy shall be used separately for trusting x/y updates. After reset, yaw integration continues from the new heading baseline.
...
The GNSS position exposed to application consumers shall be the mower control point rather than the raw antenna phase centre whenever a calibrated body-frame offset is available. That offset is fixed in mower coordinates and must be applied using the IMU heading aligned to the GNSS position's effective sample time (Pi arrival timestamp minus the receiver-claimed sample age), with wrap-safe interpolation between buffered IMU samples. The current arrival-time heading shall not be applied to a delayed position during a turn. Diagnostics shall retain the raw antenna coordinates, effective correction timestamp, correction heading, and corrected control-point coordinates.
...
An active turn shall treat the global system-stop latch as authoritative even while IMU samples continue arriving. Sensor feedback loss shall resolve the turn promptly as stopped rather than allowing healthy IMU events to keep the turn watchdog alive indefinitely. Primitive turn and line controllers shall not clear the global stop themselves; only the user-requested operation entry point may do so.
...
`TurnController` owns motor ramp-down and settling for every alignment pivot, so the drive controller shall not add a duplicate time delay. After that settled turn completes, the drive controller shall read the fused position again and recompute both remaining distance and target bearing before constructing the straight line. A small residual heading error may be accepted when its projected lateral displacement fits within the requested CTE envelope, allowing the line controller's tapered short-drive heading correction to recover it as an arc. Otherwise the drive controller shall make no more than two corrective IMU turns, each with the ordinary turn-owned settling, before starting translation.

When an operation supplies an authoritative translation-path validator, the drive controller shall invoke it from the settled live pose after every alignment pivot and again immediately before handing off to straight-line motion. A rejection shall end the manoeuvre while motor output is neutral; the controller shall not make a corrective pivot or begin translation from that rejected pose. Mowing shall supply this validator for every segment using the frozen area and obstacle geometry. A wheelbase-contained pivot may leave the fused reference point up to 25 cm beyond the recorded outer perimeter; that pose is recoverable only through a sampled, obstacle-free segment whose distance outside the area decreases monotonically to an inside target. Obstacle-interior starts and positions farther than 25 cm outside remain hard failures.

An operational straight drive may supply an authoritative CTE-reference start distinct from the live mower pose. Initial and post-turn target bearings shall still be calculated from the settled live pose to the original planned endpoint, so translation begins without trying to manufacture a new start point. Mowing-strip CTE, line heading, along-track progress and arrival geometry shall remain referenced to the immutable planned entry-to-exit baseline. A displaced start may therefore create one initial convergence arc, but it shall not redefine the remainder of the strip as a new diagonal.

Before a normal or resumed mowing-strip translation starts, the executor shall validate the settled post-pivot pose against that immutable baseline. Residual heading is compared with the configured turn-alignment tolerance and observed pivot displacement with a 15 cm anomaly threshold. Entry CTE recovery shall use a 1.5-times hysteresis around the configured segmented-drive CTE limit, so ordinary GNSS variation just outside the line controller's target does not launch a point reapproach that the immutable-baseline controller can already converge. An ordinary offset inside those limits remains the line controller's convergence task. If any recovery limit is exceeded, the executor may make one geometry-checked return to the planned entry (without the normal 15 cm mowing deadband) and one final alignment attempt. This is a bounded quality recovery rather than a separate terminal safety gate: if the second observation is still outside a quality threshold, the executor shall log the degraded entry and pass the live pose to the existing geometry-checked drive controller, which retains responsibility for another material heading realignment, translation-path rejection, and convergence onto the immutable strip baseline. It shall not require operator resume merely because a single GNSS epoch or a small turn straddles a quality threshold, and it shall not redefine the strip or regenerate the remaining plan.

Straight-line telemetry shall record signed CTE against that immutable baseline, CTE integral, distance to the target band, baseline crossings and whether crossing amplitudes decay. One initial crossing is acceptable convergence. Repeated or non-decaying crossings shall teach additional direction-specific CTE-rate damping; once damping is already substantial, persistent repeated crossing may cautiously reduce excessive proportional authority. High CTE without oscillation may still strengthen proportional correction. Persisted steering learning shall retain enough range to continue adapting while qualified mowing evidence remains outside the target band; historical parameter values shall not become artificial saturation points. The combined proportional, heading, damping and terrain correction shall remain inside the existing steering-trim ceiling, and both wheels shall remain in the selected translation direction.
...
Mowing strip sequencing shall divide the clipped free space using sweep topology. Whenever successive mowing offsets cause a mowable interval to appear, disappear, split or merge, the affected intervals shall form distinct stable regions. This decomposition shall discover visitable pockets and branches created by the outer area boundary as well as the regions around recorded obstacles. Each region shall have two complete sweep templates, one for each entry direction. The planner shall optimise the order and orientation of this small set of templates as a closed regional cycle whose last-to-first transition is also safely routable. The live mowing position shall select the nearest safely approachable region entry and rotate that fixed cycle rather than changing its transitions; after wrapping, every region is therefore completed once with the same efficient regional ordering. Within a selected region the adjacent-strip sweep remains a hard constraint. The rotated region IDs, order, strip IDs, entry/exit points, connectors and cost estimate form part of the mowing plan and shall be persisted before motion so resume continues the identical route. After the final region the mower shall return by the shortest safe route to the original GNSS position recorded when the mowing session began, and completion shall not be reported before that return succeeds.
...
Dead-reckoning calibration shall first drive a GNSS-measured straight, defaulting to 10 m, and derive independent left and right metres-per-tick values by dividing that settled GNSS distance by each wheel's encoder count. It shall then drive long forward arcs in both directions, defaulting to 180 degrees, and calculate an effective moving track width for each arc from the difference between the calibrated wheel distances divided by the IMU heading change in radians. The suggested track width shall be the mean of the available CW and CCW results. Calibration shall retain and present the raw counts and calculated values; it shall not reject an arc or claim wheel slip from a dead-reckoning endpoint residual. Material disagreement between the two directional track-width results shall be reported for operator review before applying the calibration.
...
Automatic return to charge shall be triggered only when integrated drive-motor current usage reaches the configured remaining-capacity reserve. Combined wheel travel may be accumulated and displayed as a diagnostic but shall never request recharge. An explicit operator request may still schedule recharge after the active strip.

Recharge transit shall execute the complete ordered safe route returned by the mowing transit planner. When a direct approach is unsafe, the planner shall route on the inward standoff perimeter rather than offering raw recorded area vertices as shortest-path nodes; a mathematically in-bounds route along a physical wall is not an executable transit. The complete route shall be checked against the area and obstacles before motion, and meaningful route corners shall use the same stop, standard turn, and short outgoing-edge capture as routed mowing connectors. The mower shall follow that route until the charging drive-to point becomes safely reachable, then perform the final docking line. The return-to-mow route shall meet the same requirements. Consuming a route's starting waypoint shall retain the incoming first segment as the active projection segment rather than testing the mower against a later perimeter segment.

A closed perimeter trace entered from an inward approach pose shall use that live pose only for the initial join. Any perimeter vertices skipped by an ahead join shall be rotated to the end of the route rather than discarded. After joining, the mower shall traverse every authoritative perimeter edge and close at the first joined perimeter point; it shall not replace perimeter sections with diagonal shortcuts to either the join point or the inward approach pose.

A preferred area-trace entry derived from a strip intersection shall be projected onto the authoritative cleaned area perimeter before it is inserted into the trace route. Numerical clipping residue shall not create a new exterior perimeter vertex.
...
For a selected mowing area perimeter, the page shall be able to preview logical mowing strips before motion starts. The operator can choose the strip heading, including by dragging a line on the canvas, and can choose the strip spacing. Explicitly clicking **Preview** shall clear the previously displayed and browser-stored mowing-progress trail so the generated plan is shown on a clean canvas; restoring an existing preview or editing its controls shall not clear that trail. The initial strip spacing shall be 30cm for the 40cm blade width to provide overlap. Once changed, the strip spacing shall be persisted in browser storage and restored after page refresh in preference to the default or an older area-capture value. The exact last generated preview shall also be persisted and restored with the mower's stored mowing-progress trail; restoring the page shall not regenerate or reorder the preview using the mower's current pose. When an active or resumable mowing operation has a frozen execution plan, that authoritative plan shall take precedence over the browser snapshot. The system shall clip the parallel strip lines to the selected mowing area perimeter and draw the resulting strips on the canvas.
...
Stored obstacle and area perimeter traces shall use the continuous path follower so the mower stays in smooth line-following mode once it has joined the perimeter. The complete reference polyline shall remain the execution geometry; it shall not be replaced by fitted circles or corner-cutting chords. Before motion, the complete route shall be sampled at 5cm spacing and assigned one stable lookahead between 25cm and 1 metre. The chosen value shall be the longest 5cm-increment candidate whose chord at every sampled route position remains within 5cm of the intervening reference path. The arrival section alone shall never impose the traversal lookahead. Before issuing its first forward command, the follower shall align once through `TurnController` when the live heading is outside the configured alignment tolerance of the initial moving-lookahead direction; this entry rule applies equally to new, resumed, return-to-start, recharge, and manual continuous routes. The moving target shall then advance by true ordered route distance during control while CTE remains referenced to the current path segment. Ordered projection shall search far enough forward by physical route distance to accommodate several densely sampled points without selecting a waypoint already behind the live pose. Meaningful corners shall cap lookahead at the corner. A lookahead target coincident with the current waypoint shall retain the incoming path heading and shall not create a synthetic outgoing heading. After entry alignment, the continuous steering loop shall have one behaviour only: forward differential steering toward the moving lookahead. It shall never independently request a pivot, recovery, or point-to-point segment drive because of accumulated lookahead curvature or heading error. The faster forward wheel shall remain at the requested mowing output while the inner wheel may slow continuously to zero, allowing the mower to turn about one stationary wheel without stopping. A genuine local waypoint corner identified by the execution-level corner test shall be the only mid-route stop-and-turn case; it shall delegate one outgoing-edge capture of up to 40cm to the ordinary drive controller, which owns the TurnController alignment and straight capture. Route-distance protection shall reject a gross departure beyond twice the corridor immediately, but an ordinary threshold crossing shall have to persist for ten consecutive 50Hz control samples before aborting; a single sample just beyond the threshold shall not stop mowing or start another motion mode. Wheel commands shall be rate-limited between control cycles. Verification, mowing first-encounter boundary traces, and longer perimeter-style inter-strip connectors shall reuse this follower and persist the exact ordered route and completed target index across mowing resume. A finite boundary trace shall finish when ordered progress has reached its final target and the mower has returned within the configured closed-loop detection tolerance. After tracing a boundary encountered at an unmown strip entrance, the ordinary pivot-then-straight controller shall rejoin that strip's inward standoff before strip mowing resumes. Every safe two-point inter-strip connector, regardless of length, shall use the ordinary pivot-then-straight `DriveController`; safe short multi-point connectors of at most 1 metre may also be simplified to that controller, while genuinely routed multi-point connectors retain their planner-provided path and use the aligned continuous follower.

Direct connector simplification shall be allowed only when the live-pose-to-target chord passes the same strict area and obstacle validator used immediately before translation. A chord that cuts across an area concavity shall retain or locally rebuild its routed connector. The 25cm outside-area allowance applies only when the mower is already just outside and the recovery segment moves monotonically inward.

For an execution-level corner capture of at most 40 cm, the requested CTE envelope shall be 1.5 times the ordinary segmented-drive limit, capped at the configured mowing standoff. This accommodates settled GNSS control-point movement that is a material fraction of a short capture while retaining the area-escape watchdog as the hard perimeter safety boundary.
...
Immediately before executing a planned inter-strip connector, the executor shall validate it against the same authoritative mowing area and obstacle geometry used for live safety. If that frozen connector is unsafe from the settled strip-end pose, the executor shall preserve its intended destination and make one local routing attempt from the current fused GNSS position to that destination. If continuous following subsequently aborts because the live pose has persistently left the connector corridor, the executor shall make the same single local routing attempt from the new settled pose rather than stopping the entire mowing job for a recoverable connector miss. A safe replacement shall be persisted as the active connector operation and executed without regenerating or reordering the remaining mowing plan. An unavailable, unsafe, or repeatedly failed replacement shall fail closed with the specific connector error.

A continuous follower shall reject recovery motion when the live pose is more than the minimum continuous lookahead from its ordered route and shall command neutral motor output before every successful, stopped, or failed return. When resuming a boundary trace from a live pose 25 cm to 1.5 m from the saved remainder, mowing shall discard the stale join, use the ordinary segment drive to reach a verified safe tangent-approach point, use the ordinary turn controller to align with the chosen perimeter direction, and only then start continuous following from genuine perimeter points. The displaced live pose shall never be prepended to the closed perimeter because that would create both an unsafe approach chord and a synthetic corner. When resuming an interrupted inter-strip connector from more than 25 cm away, mowing shall preserve the frozen strip order, locally plan a safety-checked transit from the current fused pose to the saved pending connector target, append the untouched remainder of that connector, persist the replacement as the active resume operation, and execute it from its first moving target. If no safe transit exists, recovery shall fail before commanding motion and expose a specific resume error. Greater boundary displacement or an unsafe boundary approach shall likewise fail before commanding perimeter-follow motor motion.
...
- **high_current** — either motor draws above the configured current threshold (currently 10 A pending validation of the installed sensors). This is treated as the mower hitting thick grass or a clump that the blades may yet cut through with another run-up.
...
- The Pi shall establish a disabled motor command during hardware initialization and refresh the latest active, neutral, or disabled command before the ESP command lease expires. Idle motion-start checks shall therefore observe a current healthy watchdog without enabling the H-bridges, while loss of the Pi heartbeat shall still expire the lease and leave drive disabled.
...
`high_current` events trigger the configured obstruction recovery. In addition, a GNSS non-progress `stall` during an active mowing-strip translation may be retried exactly once when the triggering feedback reports no sustained high current and no motor-controller fault flags. The strip drive shall first settle under the global sensor stop, then require a fresh GNSS-quality pose and revalidate the direct line from that pose to the unchanged persisted strip target against the mowing area and every obstacle. Only that specific `motor_stall_detected` stop may then be cleared before the normal turn-then-drive controller is invoked again.

A second stall on the same strip attempt, an unsafe or unavailable retry line, loss of GNSS quality, an operator or area-safety stop, a pivot stall, or a fault/high-current stall shall not be cleared by this one-shot path. `wheel_slip` and all other `stall` events shall stop the active operation and abort the session.
...
Mowing start and resume require both a fused pose whose position quality is `gnss` and an IMU heading that has been synchronised to an individually accepted absolute GNSS heading since the current pose-fusion process started. If GNSS position is unusable at start, mowing shall remain stopped with reason `poor_gnss`; if the absolute heading baseline has not yet been established, it shall remain stopped with reason `gnss_heading_not_synchronized`. Once established, heading synchronisation remains valid while the IMU owns heading between safe stationary GNSS rebases; an isolated rejected GNSS-heading epoch shall not erase that established baseline. If pose quality remains below `gnss` for 2 seconds during mowing, the runtime shall request a global stop with reason `poor_gnss`. Brief validator transitions shorter than that debounce do not stop an otherwise healthy mow. This gate applies to autonomous mowing only.
...
Disconnected sweep regions shall be ordered as the open route that the mower will actually execute from its selected entry. Transition scoring shall use a bounded geometric distance plus a penalty when direct travel is unavailable, so region ordering remains responsive for complex lawns. Complete safe connector geometry shall then be constructed and validated for every selected transition; an unsafe selected route shall fail planning. The planner shall not optimise an artificial closed regional cycle and then rotate that cycle to the entry point.
...
For Carry On, trusted GNSS means both trusted fused position and an accepted absolute GNSS heading baseline established during the current pose-fusion process; checking position quality alone is insufficient.
...
When mowing stops or errors after an operation has begun, the Drive & Paths page shall offer a **Carry On Mowing** control. Carry On shall be rejected before clearing the global stop unless fused pose quality is trusted GNSS and motor feedback has produced at least three consecutive coherent frames, the newest no more than 250 ms old, with no watchdog failure or motor fault flags. The saved state shall include the active operation, its exact ordered route and, for continuous boundary or connector following, the completed target index so resumption continues from the interrupted ordered path position rather than restarting the saved execution geometry. An interrupted mowing-strip translation is the exception: if the mower has not reached that strip's exit target, Carry On shall first return to the frozen plan's entry standoff for the current strip, then turn and mow that strip again from start to finish before continuing the unchanged strip sequence. The control shall become available after the active executor has settled and shall remain available across process restarts while the saved state exists. A temporary failure of any initial page request shall not prevent later mowing-status polling or leave Carry On permanently hidden, and Start/Carry On actions shall allow only one request at a time. If the resumed GNSS pose is outside the mowing area by no more than 75 cm, Carry On shall first find the nearest perimeter point, construct a target at least 25 cm inside the area, reject a recovery line that intersects an obstacle, pivot toward that target through the ordinary drive controller, and drive into the area before continuing the saved operation. A resumed pose materially inside a recorded obstacle shall fail closed with the obstacle index and live coordinates logged so the operator can carry the mower into mapped free space. The sole automatic exception is a fused-control-point offset no more than 25 cm inside one obstacle after a wheelbase-contained pivot: the executor may follow only the unchanged saved point-to-point target when that target is outside every obstacle, 5 cm samples of the complete line remain inside the mowing area, avoid every other exclusion, monotonically reduce penetration into the source obstacle until clear, and the settled forward heading itself immediately reduces that penetration. This same bounded check applies immediately after a mowing-strip entry pivot, so a mower already facing safely out may proceed without an artificial stop-and-resume cycle. A deeper start, a tangential or initially inward heading, a line that moves farther inside, re-entry, another obstacle, or any unsafe target shall remain stopped. After outside-area recovery, this bounded obstacle-offset recovery, or operator relocation, any saved point-to-point operation shall be checked from the new live pose: a direct line that leaves the area or intersects an obstacle shall be replaced by a safety-checked, persisted mowing transit route rather than issuing one unconstrained straight drive to the stale target. An unsafe live pose, a farther outside pose, unavailable safe target, blocked or unsafe recovery route, or failure to enter the polygon shall fail closed without resuming mowing.

During each mowing-strip translation the executor shall retain a spatially sampled trace of trusted fused positions. After a strip completes, consecutive strips in the same sweep region shall be compared along their shared mowing axis. If the perpendicular separation of the two actual traces exceeds the plan's cutter width for at least 20 cm, the executor shall create a temporary line whose corresponding endpoints are the midpoints of the two actual driven traces. It shall mow that bisector in the previous strip's direction, then re-mow the current strip in its original direction before proceeding. The repair sequence and its line anchors shall be resumable, and each adjacent pair shall receive no more than one automatic repair.
