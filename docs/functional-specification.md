...
The same page shall also support recording one or more named mowing area perimeters. A mowing area perimeter is captured from position samples every 10cm, is persisted separately from obstacle perimeters, and defaults to a generated mowing-area name when the operator does not provide one. The live canvas shall auto-scale to show the current position history, stored obstacle perimeters, and stored mowing area perimeters together. The mowing-progress trail shall remain at constant opacity and persist across page refreshes and **Carry On Mowing** operations. It shall be cleared only after a new **Mow Area** or **Mow From Perimeter** request starts successfully. A large position discontinuity shall break the rendered trail rather than erase previously recorded progress or draw a false connecting segment.
...
Stored obstacle and area perimeter traces shall use the continuous path follower so the mower stays in a smooth line-following mode once it has joined the perimeter. Before motion, the smoothed reference polyline shall be greedily fitted into committed straight and circular-arc primitives. From each retained endpoint the fitter shall test progressively farther reference points, accept a straight chord only while every intervening point remains within the configured 5cm simplification tolerance, and accept a circular arc only while radial deviation, turn direction, local turn size, and ordered arc sweep remain smooth within that same tolerance. The primitive reaching the furthest valid reference point wins, with a straight preferred on an equal endpoint. A failed candidate ends the current primitive so the fitter cannot search past a real corner. Straight primitives collapse to their endpoints; arc samples are projected onto their fitted circle and remain continuous reference geometry rather than individual stop targets. A fitted arc shall be executed as one committed curvature derived from its radius, direction, and the calibrated wheelbase, with bounded radial and tangent-heading corrections; its projected reference samples shall advance ordered progress but shall not independently trigger pivot or recovery decisions. Perimeter following shall use a capped steady speed on straight and gentle spans, stop lookahead at primitive corners, and approach within 15cm of the waypoint before locking the outgoing edge and pivoting once to align with it. After alignment the follower shall retain that outgoing edge and complete a committed straight capture of up to 40cm before path projection and ordinary cross-track steering resume; it shall not reverse the pivot direction during that capture. A tight requested non-fitted curve that would reverse or nearly stop the inner wheel shall pivot instead. Every such recovery pivot shall lock its selected path segment and turn direction until aligned, then retain that segment for up to 40cm of forward capture before path projection resumes. Commitment state transitions shall always yield to sensor and stop processing rather than forming a zero-time controller loop. Wheel commands shall be rate-limited between control cycles, and pivot/arc mode shall use separate entry and exit thresholds, so controller corrections do not repeatedly alternate between fast, slow, and pivot outputs near a threshold. Verification, mowing first-encounter boundary traces, and longer perimeter-style inter-strip connectors shall reuse that same continuous follower and persist the fitted execution geometry and complete perimeter-control profile across mowing resume. A finite boundary trace shall finish when ordered progress has reached its final target and the mower has returned within the configured closed-loop detection tolerance; it shall not force the mower around a tiny closing chord. After tracing a boundary encountered at an unmown strip entrance, the ordinary pivot-then-straight controller shall rejoin that strip's inward standoff before strip mowing resumes. An inter-strip connector whose target is less than or equal to 1 metre from the live mower pose shall instead use the ordinary pivot-then-straight segment controller when the direct segment remains inside the mowing area or no more than 25cm outside it and does not intersect an obstacle. Geometrically unsafe short connectors shall retain their planner-provided routed path.
